<section id="about" class="section bg-light-secondary dark:bg-dark-secondary" >
    <div class="container mx-auto">
        <div class="flex flex-col xl:flex-row gap-24">
            <img class="object-cover h-full w-[566px] md:mx-auto lg:mx-0 rounded-2xl"
            src="/img/about-img.jpeg" alt="About img">
            <div class="flex flex-col items-center text-center lg:items-start lg:text-left">
                <div class="flex flex-col">
                    <h2 class="text-3xl lg:text-4xl font-medium lg:font-extrabold mb-3">About me</h2>
                    <p class="mb-4 text-accent">Developer Student
                    </p>
                    <hr class="mb-8 opacity-90 dark:opacity-5"/>
                    <p class="mb-8">
                        I'm from Gloriosa UAGRM, in fact I'm studying around 5 years ago and always
                        I like computers!<br />
                        I want to work in NASA1
                    </p>
                </div>

                <a href="/contact" class="btn btn-md bg-accent text-light-primary">
                        Work with me.
                </a>
            </div>
        </div>
    </div>
</section>
