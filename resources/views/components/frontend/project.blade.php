...
<a x-data="{skill:{{ json_encode($project->skill) }}}"
    href="{{ $project->project_url }}" target="_blank" :class="selectedTab == 'all' || selectedTab == skill.id ? 'block' : 'hidden'"
    ...
    <div class="px-6 py-4">
        ...
        group-hover:text-light-tail-100 dark:group-hover:text-dark-navy-500">
            {{ $project->name }}
        </h1>
